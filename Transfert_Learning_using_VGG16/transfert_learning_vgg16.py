...
# convert class labels to on-hot encoding like [1 0 0 0] for the first
# class [0 1 0 0] for the second classe
...
# We have only 4 classes for our custom dataset, so we need to replace the
# last layer of the VGG16 model (preduction Dense)
...
# Freeze the model like the method 3 in transfert learning : to check you
# need to see the number of non trainbel parameters change from zero to
# other value in model.summary
...
#----------------------------------Method 2 Use the fine tunnig for a larger dataset--------------------#
print('------------------------Method 2----------------')
...
# freez the layers excepet the 3 last layer (which are modified in the
# custom_vgg_model2)
...
t = time.time()
hist = custom_vgg_model2.fit(X_train, y_train, batch_size=32, epochs=10, verbose=1, validation_data=(
    X_test, y_test))  # if we have valdation set we can repalce (X_test, y_test) by (X_validation, y_validation)
print('Training time: %s' % (t - time.time()))
(loss, accuracy) = custom_vgg_model2.evaluate(
    X_test, y_test, batch_size=10, verbose=1)
print("[INFO] loss={:.4f}, accuracy: {:.4f}%".format(loss, accuracy * 100))
